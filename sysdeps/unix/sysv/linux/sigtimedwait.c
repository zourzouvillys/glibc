...
# endif
    ...
    /* XXX The size argument hopefully will have to be changed to the
       real size of the user-level sigset_t.  */
  struct kernel_timespec64 ts64, *tsp64;
  if (timeout)
  {
  ...
  int result = SYSCALL_CANCEL (rt_sigtimedwait, set, info, tsp64, _NSIG / 8);

  if (result == -1 && errno == ENOSYS)
  {
    struct kernel_timespec32 ts32, *tsp32;
    if (timeout)
    {
      if (timeout->tv_sec < INT32_MIN || timeout->tv_sec > INT32_MAX)
      {
        errno = EOVERFLOW;
        return -1;
      }
      ...
      tsp32 = NULL;
    result = SYSCALL_CANCEL (rt_sigtimedwait64, set, info, tsp32, _NSIG / 8);
  }
